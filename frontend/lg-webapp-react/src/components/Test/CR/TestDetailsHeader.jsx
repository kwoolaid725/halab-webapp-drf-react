...
function TestDetailsHeader (props) {
  ...
  const testStatusLabel = TEST_STATUS.find(
    status => status.value === props.testStatus)?.label
  ...
  const handleButtonSubmit = () => {
    ...
    const newCompletionDate = isCompletedChecked
      ? formattedDate
      : props.completionDate

    axiosInstance.patch(`admin/tests/edit/${props.testId}/`, {
      test_status: newTestStatus, remarks, completion_date: newCompletionDate,
    }).then((res) => {
  ...
  return (
    <ThemeProvider theme={theme}>
      ...
      <Grid container spacing={2} sx={{ marginBottom: '10px' }}>
        {/* Left Panel */}
        <LeftPanel item xs={12} sm md="auto" xl>
        ...
        {/* Right Panel */}
        <RightPanel item xs={12} sm md xl>
          {/* Remarks header */}
          <TableRow>
            <StyledTableCell
              sx={{
      ...
      <Grid container spacing={2} sx={{ marginTop:'-30px', marginBottom: '40px' }}>
        <Grid item xs={12}>
          <Box sx={{
            border: '1px solid #ffffff', padding: '10px', display: 'flex',
            ...
            <Button
              ...
              startIcon={<UpdateIcon style={{
                fontWeight: 'bold', fontSize: '24px', color: '#38a3a5',
              }}/>}
              variant="contained"
              color="primary"
              style={{
                fontWeight: 'bold', fontSize: '16px',
                backgroundColor: '#f9f9f9', color: '#38a3a5',
                border: '1px solid #38a3a5', marginLeft: 'auto', // Move button to the right
              }}
            >
              Submit Changes
            </Button>
